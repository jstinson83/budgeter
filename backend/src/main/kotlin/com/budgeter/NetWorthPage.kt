...
private fun scenarioRowModel(scenario: Scenario, categoryLabelById: Map<String, String>, index: Int): Map<String, Any?> = mapOf(
    ...
    "salaryChangeEndDate" to (scenario.salaryChangeEndDate?.toString() ?: ""),
    "salaryChangeRrspContributionOverride" to (scenario.salaryChangeRrspContributionOverride?.let { "%.2f".format(it) } ?: ""),
    "salaryChangeMarginalTaxRateOverridePercent" to (scenario.salaryChangeMarginalTaxRateOverride?.let { "%.1f".format(it * 100) } ?: ""),
    "salaryChangeRoomAccrualOverride" to (scenario.salaryChangeRoomAccrualOverride?.let { "%.2f".format(it) } ?: ""),
    "hasRrspStrategy" to (scenario.rrspMonthlyContribution != null),
